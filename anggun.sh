...
FIGHTERTUNNEL() {
    curl -sS https://raw.githubusercontent.com/remooot/permission/main/ip >/root/tmp
...
ftTunneling() {
    ...
    IZIN=$(curl -sS https://raw.githubusercontent.com/remooot/permission/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
        TIMEDATE
    else
